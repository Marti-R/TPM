...
def experiment_loop(instruction_pipe, settings, simulation=False, target_fps=60, show_fps=True):
    try:
        os.nice(-20)
    ...
    timestamp_volt = time.perf_counter()
    ...
    while True:
        ...
        if time.perf_counter() < trial_start_time + 1.4:
            if int((time.perf_counter() - trial_start_time) / 0.2) % 2 != 0:
        ...
        if simulation:
            ...
            position_volt += virtual_speed * (1. * (timestamp_volt - last_frame))
        ...
        update_rectangle_list.extend(experiment_screen.move_and_blit(int(shift)))
